...
## Компактные параметры на mobile

На экранах до 600 px существующие selectors находятся в одной disclosure-панели `Параметры` с кратким summary. До первого Start и во время настройки следующего отрезка панель раскрыта; после Start, при running/paused и после восстановления PWA — свернута. Кнопка использует `aria-expanded` и `aria-controls`; второго набора selectors нет. На desktop текущая панель остаётся постоянно раскрытой.
